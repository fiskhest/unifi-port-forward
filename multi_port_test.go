...
func TestMultiPortService_ValidAnnotation(t *testing.T) {
	...
	handlers.ClearPortConflictTracking()
	...
	portConfigs, err := handlers.GetPortConfigs(service, "kube-port-forward-controller/ports")
	if err != nil {
	...
	for _, pc := range portConfigs {
		portName := strings.TrimPrefix(pc.Name, "multi-service-")
		expectedPort, exists := expectedMappings[portName]
		if !exists {
			t.Errorf("Unexpected port name: %s", portName)
		}
...
func TestServiceWithoutAnnotation_Skipped(t *testing.T) {
	...
	_, err := handlers.GetPortConfigs(service, "kube-port-forward-controller/ports")
	if err == nil {
...
func TestPortConflictDetection_Error(t *testing.T) {
	...
	_, err1 := handlers.GetPortConfigs(service1, "kube-port-forward-controller/ports")
	if err1 != nil {
	...
	_, err2 := handlers.GetPortConfigs(service2, "kube-port-forward-controller/ports")
	if err2 == nil {
...
func TestDefaultPortMapping(t *testing.T) {
	...
	for _, pc := range portConfigs {
		portName := strings.TrimPrefix(pc.Name, "default-service-")
...
// Helper function to get service port by name
